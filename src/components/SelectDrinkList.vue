<template>
  <div class="selectDrinkList-warp">
    <div class="info-wrap">
      <h3 class="font-bold">마시고 싶은 음료를 골라주세요</h3>
    </div>
    <div class="content-wrap">
      <ul class="content-list">
        <li v-for="item in searchDrink(getDrinkType)" :key="item.name_en">
          ...
          <button
            ...
            @click.prevent="selectDrink(item.name)"
          >
            <span>선택</span>
          </button>
        </li>
...
export default {
  ...
  methods: {
    ...
    selectDrink(select) {
      this.setDrink(select);
      this.$router.push("oderlist");
    },
    searchDrink: function (type) {
      this.displayList = this.getDrinkList.filter((item) => item.type == type);
      return this.displayList;
    },
  },
  computed: {
    ...mapGetters(store, {
      getDrinkType: "getDrinkType",
    }),
    ...mapGetters(product, {
